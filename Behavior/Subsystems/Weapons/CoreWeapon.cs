	...
	public class CoreWeapon : BaseWeapon, IWeapon{
		...
		private void StaticWeaponReadiness() {
			...
			if (badCollisionResult) {
				...
				//Logger.MsgDebug(" - Bad Collision Result", DebugTypeEnum.Weapon);
				_readyToFire = false;
				return;
			...
			if (badAngleAndNoHostileCollision || beamAmmoAndNoHostileCollision) {
			...
			if (!hostileCollisionResult) {
				...
				if (dist > trajectory || distFromMaxTrajectory > _weaponSystem.WeaponMaxBaseDistanceTarget) {
					...
					//Logger.MsgDebug(" - Bad Distance", DebugTypeEnum.Weapon);
					_readyToFire = false;
					return;
				...
				if (collision.GetCollisionDistance() > trajectory) {
					...
					//Logger.MsgDebug(" - Bad Distance With Collision", DebugTypeEnum.Weapon);
					_readyToFire = false;
					return;
